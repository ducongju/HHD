...
class UBTech:
    ...
    def __init__(self, vid, pid):
        self.vid = vid
        self.pid = pid
        UBTech.h.open(self.vid, self.pid)
    ...
    def controlSigleServo(self, id, angle, time, timeout):
        arg = [hex(id), hex(angle), hex(int(time*50)),
               hex(int(timeout/20)//256), hex(int(timeout/20) % 256)]
    ...
    def setSingleServoOffset(self, id, offset):
        offset = offset + 65536
        self.__sendCmd(COMMAND.setSingleServoOffset, [
                       hex(id), hex(int(offset)//256), hex(int(offset) % 256)])
    # 设置多个舵机偏移值
    # 读取单个舵机偏移值
    ...
    def __sendCmd(self, cmd, paras=['0x00']):
        result = [hex(0x00), hex(0xfb), hex(0xbf)]
        cmdLen = len(paras)+5
        chk = cmdLen
