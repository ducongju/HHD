...
class HRNet_teather(BaseBackbone):
    """HRNet backbone.
    ...
    def init_weights(self):
        ...
        super(HRNet_teather, self).init_weights()
    ...
    def forward(self, x):
        """Forward function."""
        x_small=nn.functional.interpolate(x, scale_factor=0.5)
        x_small=self.conv1(x_small)
        x_small = self.norm1(x_small)
        x_small = self.relu(x_small)
        x_small = self.conv2(x_small)
        x_small = self.norm2(x_small)
        x_small = self.relu(x_small)
        x = self.conv1(x)
        x = self.norm1(x)
        x = self.relu(x)
        ...
        for i in range(self.stage2_cfg['num_branches']):
            if self.transition1[i] is not None and i!=1:
