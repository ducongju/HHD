...
class IntegralRegressionHead(BaseHead):
    ...
    def forward(self, feats: Tuple[Tensor]) -> Union[Tensor, Tuple[Tensor]]:
        ...
        #         feature = heatmaps[:, i, :, :] # 在channel维度上，每个channel代表了一个卷积核的输出特征图，所以对每个channel的图像分别进行处理和保存
        #         feature = feature.view(feature.shape[1], feature.shape[2]) # batch为1，所以可以直接view成二维张量
        #         feature = feature.cpu().detach().numpy() # 转为numpy
        #         with open('/data-8T/lzy/mmrazor/data/test/feature_model.txt', 'a') as fp:
        ...
        #         cv2.imwrite('/data-8T/lzy/mmrazor/data/test/' +str(self.flag)+'-'+ str(i) + '_model.jpg',feature)  # 保存当前层输出的每个channel上的特征图为一张图像
        # self.flag=1
